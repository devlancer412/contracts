...
contract QBuxVault is Ownable {
    ...
    event Deposit(address indexed account, uint256 indexed timestamp, uint256 usdValue, uint256 value);
    event Withdraw(address indexed account, uint256 indexed timestamp, uint256 usdValue, uint256 value);
    ...
    function _validRedeemParam(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(account, value, timestamp)
        );
    ...
    constructor(IERC20 _erc20token, address _authorizer, uint256 _exchange_rate) {
    ...
    function deposit(uint256 value_token) public {
        ...
        erc20token.transferFrom(msg.sender, address(this), value_token);
